...
class ProductDocumentsController extends Controller
{
    ...
    {
        $parameters = ProductVideos::where('product_id', $product_id)->get();

        if (!$parameters) abort(404);

        return view('cp.product_documents.index', compact('parameters', 'product_id'))->with('title', 'Список документации');
    }
    ...
    {
        return view('cp.product_documents.create_edit', compact('product_id'))->with('title', 'Добавление документации');
    }
